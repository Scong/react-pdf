...
var resolveAssets = /*#__PURE__*/ (function() {
  var _ref = _asyncToGenerator__default['default'](
    /*#__PURE__*/ _regeneratorRuntime__default['default']().mark(
      function _callee(node, fontStore) {
        var promises;
        return _regeneratorRuntime__default['default']().wrap(function _callee$(
          _context,
        ) {
...
var warnFallbackSpace = function warnFallbackSpace(node) {
  console.warn(
    'Node of type ' +
      node.type +
      " can't wrap between pages and it's bigger than available page height, falling back to wrap",
  );
};
var breakableChild = function breakableChild(children, height, path) {
  if (path === void 0) {
    path = '';
  }
  for (var i = 0; i < children.length; i += 1) {
    if (children[i].type === 'TEXT_INSTANCE') continue;
    if (shouldBreak(children[i], children.slice(i + 1, height))) {
      return {
        child: children[i],
        path: path + '/' + i,
      };
...
var splitByFirstChildBreak = function splitByFirstChildBreak(
  firstBreakableChild,
  currentNode,
  currentPath,
  height,
) {
  var preBreakChildren = [];
  var postBreakChildren = [];
  var nextIndex = Number(currentPath.split('/').pop());
  var _splitNode = splitNode(currentNode, height),
    preBreakNode = _splitNode[0],
    postBreakNode = _splitNode[1];
  for (var i = 0; i < currentNode.children.length; i += 1) {
    // force recompute of lines on relayout.
    // as the structure has changed.
    var subjectNode = Object.assign({}, currentNode.children[i], {
      lines: null,
    });
    if (i < nextIndex) {
      ...
      if (currentPath === firstBreakableChild.path) {
        var theBrokenChild = subjectNode;
        var props = Object.assign({}, theBrokenChild.props, {
          wrap: true,
          break: false,
        });
        var next = Object.assign({}, theBrokenChild, {
          props: props,
        });
        postBreakChildren.push(next);
      } else {
        var _splitByFirstChildBre = splitByFirstChildBreak(
            firstBreakableChild,
            subjectNode,
            currentPath +
          ...
          nestedPreBreakChild = _splitByFirstChildBre[0],
          nestedPostBreakChild = _splitByFirstChildBre[1];
...
var splitNodes = function splitNodes(height, contentArea, nodes) {
  ...
  for (var i = 0; i < nodes.length; i += 1) {
    ...
    if (shouldBreak$1) {
      ...
      var _props = Object.assign({}, child.props, {
        wrap: true,
        break: false,
      });
    ...
    var firstBreakableChild =
      child.children &&
      child.children.length > 0 &&
      breakableChild(child.children, height);
    if (firstBreakableChild) {
      var _splitByFirstChildBre2 = splitByFirstChildBreak(
          firstBreakableChild,
          child,
          firstBreakableChild.path
            .split('/')
        ...
        currentPageNode = _splitByFirstChildBre2[0],
        nextPageNode = _splitByFirstChildBre2[1];
      var _box2 = Object.assign({}, nextPageNode.box, {
        top: nextPageNode.box.top - height,
      });
      var _next3 = Object.assign({}, nextPageNode, {
        box: _box2,
      });
      if (currentPageNode) currentChildren.push(currentPageNode);
      nextChildren.push.apply(nextChildren, [_next3].concat(futureNodes));
      break;
    }
    if (shouldSplit) {
      ...
      if (child.children.length > 0 && currentChild.children.length === 0) {
        var _box3 = Object.assign({}, child.box, {
          top: child.box.top - height,
        });
        var _next4 = Object.assign({}, child, {
          box: _box3,
        });
        currentChildren.push.apply(currentChildren, futureFixedNodes);
        nextChildren.push.apply(nextChildren, [_next4].concat(futureNodes));
        break;
      }
...
var splitView = function splitView(node, height, contentArea) {
  var _splitNode2 = splitNode(node, height),
